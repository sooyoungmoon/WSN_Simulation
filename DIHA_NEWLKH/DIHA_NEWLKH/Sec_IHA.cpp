...
void Sec_IHA::setMAC (EvntMsg* msg)
{
...
void Sec_IHA::setMAC (EvntMsg* msg, int clstrID) // (2014.08.20)
{
...
bool Sec_IHA::verMAC(EvntMsg* msg) // (2013.08.20)
{
	bool vResult = true;
...
void Sec_IHA::updateMAC(struct _tagEvntMsg* msg) // (2014.08.20)
{
...
void Sec_IHA::doEventMsgProc(struct _tagEvntMsg* msg) // (2014.08.21) 수신된 이벤트 보고서 처리
{	
...
void Sec_IHA::writeEventMsg(ofstream& out, int event_no, struct _tagEvntMsg* msg) // (2014.08.11) 생성된 이벤트 보고서 테스트 (파일에 기록)
{
...
void Sec_IHA::genCntrlMsg(ctrlMsgType type, BS* bs) // (2014.05.20 추가) BS에서 제어 메시지 생성
{
	if (bs == NULL)
	{
		cout << "(bs == NULL)" << endl;
...
void Sec_IHA::genCntrlMsg(ctrlMsgType type, Node* cNode) // (2013.08.23 추가) 노드에서 제어 메시지 생성
{
	if (cNode == NULL)
	{
...
void Sec_IHA::testCntrlMsg() // 제어 메시지 테스트 함수 재정의 (2013.09.09 추가)
{
...
void Sec_IHA::doCntlOp (struct _tagCntrlMsg* msg, Node* cNode) // (2014.08.18 수정)
{
	// 제어 메시지 처리 과정 구현
...
int Sec_IHA::getEventMsgLen(struct _tagEvntMsg* msg)
{
	int eventMsgLen = 0;

	eventMsgLen = DAT_MSG_LEN + (INTERLEAVING_T+1) * PMAC_LEN; // 기본 데이터 메시지 + (T+1) 개의 MAC

	return eventMsgLen; // IHA의 이벤트 보고서 길이 반환
}

int Sec_IHA::getCntrlMsgLen(struct _tagCntrlMsg* cm) // (2014.01.10) 제어 메시지 길이 반환
{
	int cntrlMsgLen = 0;
...
bool Sec_IHA::detect(EvntMsg* msg) // (2013.08.20)공격이 검출되면 TRUE, 아니면 false 반환
{
...
void Sec_IHA::updateAssocNodes(int nodeID, int assocType, int hopDist, set<int> assocNodes) // assocType - 0: upper node,1 : lower node
{
	switch (assocType)
	{
...
bool Sec_IHA::compareAssoc ( set<int> oldAssoc, set<int> newAssoc) // (2014.05.27) 두 연관 노드 목록이 동일하며 true,  다르면 false 반환
{
	bool res = true;
...
void Sec_IHA::resetAssoc() // (2014.08.21) 이전 라운드의 연관 노드 정보를 리셋
{
	upperNodes.clear();
	lowerNodes.clear();
}
...
bool Sec_IHA::hasUpdatedAssoc (int nodeID) // (2014.07.17) 특정 노드의 연관 노드 변경 여부 반환
{
...
void Sec_IHA::resetUpdatedAssoc() // (2014.07.22) 노드들의 연관 노드 갱신 여부를 false 로 리셋 (키 재분배 목적)
{
...
void Sec_IHA::testAssoc() // // (2014.07.17) 노드들의 연관 노드 목록 파일에 출력
{
