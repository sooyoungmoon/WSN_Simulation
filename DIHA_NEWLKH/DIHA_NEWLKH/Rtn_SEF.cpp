...
list<Node*> Rtn_SEF::selNext (struct _tagCntrlMsg* cntrlMsg, BS* bs) // // 제어 메시지를 위한 다음 노드 선택
{
...
list<Node*> Rtn_SEF::selNext (struct _tagCntrlMsg* cntrlMsg, Node* cNode)  // 제어 메시지를 위한 다음 노드 선택
{
	list<Node*> nextNdLst;
...
Node* Rtn_SEF::selNext(struct _tagEvntMsg* evntMsg, Node* cNode) // (2014.08.13) 이벤트 보고서를 위한 다음 전달 노드 결정
{
	// 최단 경로 기반 라우팅

	Node* next = NULL;

	Node* spParent = this->getSPParentNode(cNode); // 현재 노드의 최단 경로 부모 (비 CH) 노드 반환
...
list<Node*> Rtn_SEF::selNext (struct _tagKeyMsg* keyMsg, BS* bs)
{
	list<Node*> nextNdLst;
	set<Node*> nextNdSet;
...
list<Node*> Rtn_SEF::selNext (struct _tagKeyMsg* keyMsg, Node* cNode)
{
	list<Node*> nextNdLst;
	set<Node*> nextNdSet;
...
list<Node*> Rtn_SEF::selNext (struct _tagTREQ* treq, Node* cNode) // (2014.05.10) TREQ 메시지 전달을 위한 다음 노드 선택
{
	// 구현 중
...
list<Node*> Rtn_SEF::selNext (struct _tagTRES* tres, Node* cNode) // (2014.05.10) TRES 메시지 전달을 위한 다음 노드 선택
{
	// 구현 중
...
double Rtn_SEF::eval (struct _tagCntrlMsg* cntrlMsg, Node* candi)
{
	double res = 0; // 후보 노드의 평가 결과
...
double Rtn_SEF::eval (struct _tagEvntMsg* evntMsg, Node* candi)
{
	double res = 0; // 후보 노드의 평가 결과
...
double Rtn_SEF::eval(Node* recipent, Node* candi) // 키 재분배 메시지 전달을 위한 후보 노드 평가
{
	double res = 0;
...
double Rtn_SEF::testConect()
{
	map<int, Node*>::iterator it;

 	int numNodesInClstr = 0;
	int numIsolatedNodes = 0; // BS까지 연결되지 못한 ch
	...
	for (it = nodes.begin(); it != nodes.end(); it++)
	{
		...
		{
			tempNode = this->getSPParentNode(tempNode); 				
	...
	res =  ((double) numIsolatedNodes)/ nodes.size();
